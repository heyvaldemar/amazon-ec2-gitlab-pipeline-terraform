...
resource "aws_s3_bucket" "bucket_1" {
  ...
  bucket              = "terraform-state-storage-gitlab-1-${random_id.random_id_1.hex}"
  object_lock_enabled = true

  tags = {
    Name = "terraform-state-storage-gitlab-1"
  }
...
resource "aws_s3_bucket" "log_bucket_1" {
  bucket              = "terraform-state-storage-logs-gitlab-1-${random_id.random_id_1.hex}"
  object_lock_enabled = true
}
...
resource "aws_s3_bucket" "log_bucket_2" {
  bucket              = "vpc-flow-logs-gitlab-1-${random_id.random_id_1.hex}"
  object_lock_enabled = true
}
