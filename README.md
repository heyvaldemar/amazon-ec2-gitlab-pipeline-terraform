...
Every release from v1.1.4 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `amazon-ec2-gitlab-pipeline-terraform-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `amazon-ec2-gitlab-pipeline-terraform-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `amazon-ec2-gitlab-pipeline-terraform-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob amazon-ec2-gitlab-pipeline-terraform-<tag>.tar.gz \
  --bundle amazon-ec2-gitlab-pipeline-terraform-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/amazon-ec2-gitlab-pipeline-terraform/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact amazon-ec2-gitlab-pipeline-terraform-<tag>.tar.gz \
  --provenance-path amazon-ec2-gitlab-pipeline-terraform-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/amazon-ec2-gitlab-pipeline-terraform
```
...
**What the configuration promises is tested, not just parsed.** [`tests/posture.tftest.hcl`](tests/posture.tftest.hcl) plans the configuration with its default variables against mocked providers, with no AWS account and no credentials involved, and makes 32 assertions about what that plan would build: KMS keys rotate, no bucket can be made public, buckets are encrypted with KMS and versioned, the state lock table is encrypted and recoverable to a point in time, and each resource type below it keeps its own promise. [`tests/plant_violations.py`](tests/plant_violations.py) then breaks those promises one at a time on a copy of the configuration, 18 ways listed in [`tests/plants.tsv`](tests/plants.tsv), and fails the run if the test stays green through any of them. Both run in CI on every push.
...
**What the defaults do not promise, stated rather than hidden.** The test asserts only what the defaults deliver; these are the places they do not, and each is a decision to make before `apply`:

- **Redis is not encrypted at rest** by default.
- **Redis is not encrypted in transit** by default; enabling it means the application has to speak TLS to it.
- **SSH is open to the internet.** The instance's security group accepts port 22 from `0.0.0.0/0`. Narrow it to your own address before `apply`, or remove the rule and reach the host through AWS Systems Manager.
